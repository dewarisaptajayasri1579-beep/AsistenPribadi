...
async function main() {
  ...
  try {
    ...
    ok("agenda yang selesai ikut dihitung", /Agenda: 1 dari 1/.test(pesan))
    ok("tidak lagi melaporkan '0 selesai'", !/0 selesai/.test(pesan))
    ok("tugas jatuh tempo minggu depan tidak disebut 'belum hari ini'", !/1 masih menunggu/.test(pesan))
  } finally {
